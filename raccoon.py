...
def LoadLogWindow(logname=None):
    ...
    DisableInterface()
    if not logname:
        logname = askopenfilename(parent=root, title="Select a Raccoon session file to load...",
                                  filetypes=[("Raccoon Session File", "*.rsf"), ("old Raccoon 1.0 Log file", "*.log"),
                                             ("Any file...", ("*"))])
    if not logname:
        EnableInterface()
        return False
    ...
    Label(LoadOptWin, text="Select data to import:", justify=LEFT).grid(row=1, column=0, columnspan=3, sticky=N, padx=5,
                                                                        pady=15)
    l_frame = Pmw.Group(LoadOptWin, tag_text="Ligands")
    Label(l_frame.interior(), text="                                        ").grid(row=2, column=0,
                                                                                    sticky=W)  # Tk sucks: workaround...
    Checkbutton(l_frame.interior(), text="Ligands list", variable=LoadLig).grid(row=2, column=0, columnspan=1, sticky=W)
    Checkbutton(l_frame.interior(), text="Filter set", variable=LoadFilter).grid(row=3, column=0, columnspan=1,
                                                                                 sticky=W)
    l_frame.grid(row=3, column=0, columnspan=3, sticky=W + E, padx=5, pady=5)

    r_frame = Pmw.Group(LoadOptWin, tag_text="Target")
    Label(r_frame.interior(), text="                                        ").grid(row=4, column=0,
                                                                                    sticky=W)  # Tk sucks: workaround...
    Checkbutton(r_frame.interior(), text="Receptors list", variable=LoadRec).grid(row=4, column=0, columnspan=1,
                                                                                  sticky=W)
    Checkbutton(r_frame.interior(), text="Flexible residues", variable=LoadFlex).grid(row=5, column=0, columnspan=1,
                                                                                      sticky=W)
    r_frame.grid(row=4, column=0, columnspan=3, sticky=W + E, padx=5, pady=5)

    p_frame = Pmw.Group(LoadOptWin, tag_text="Parameters")
    Label(p_frame.interior(), text="                                        ").grid(row=6, column=0,
                                                                                    sticky=W)  # Tk sucks: workaround...
    Checkbutton(p_frame.interior(), text="Map parameters", variable=LoadMap).grid(row=6, column=0, columnspan=1,
                                                                                  sticky=W)
    Checkbutton(p_frame.interior(), text="Docking parameters", variable=LoadDock).grid(row=7, column=0, columnspan=1,
                                                                                       sticky=W)
    ...
    Button(LoadOptWin, text="Cancel", command=lambda: LoadOptWin.destroy()).grid(row=10, column=2, sticky=W, padx=10,
                                                                                 pady=10)
    EnableInterface()
...
def LoadLog(logname):
    ...
    if LoadFilter.get():
        try:
            for index in range(38, 44):
                ...
                if DEBUG: print
                "LOAD_SESSION: filter params => ", logfile[index]
            # Hb donor max-min
    ...
    if LoadRec.get():
        ...
        try:
            for line in logfile:
                if line[0:7] == "TARGET>":
                    ...
                    if DEBUG: print
                    "LOAD_SESSION> found receptor: |%s|" % receptor_list[0]
        except:
        ...
        if len(receptor_list) > 1:
            ...
            try:
                ...
                if (tkMessageBox.askokcancel("Warning", ((
                                                         "Error while loading the receptor structure:\n %s\n\nSkip receptor loading and continue loading the log file?") %
                                                         receptor_list[0]))) == 0:
                    return False
        ...
        try:
            for line in logfile:
                if line[0:5] == "FLEX>":
                    ...
                    if "file" in line:
                        if DEBUG: print
                        "LOAD_SESSION: got flex from file", line.split(":", 1)[1][:-1].split("\t")[1]
                    ...
                    if "selection" in line:
                        if DEBUG: print
                        "LOAD_SESSION: got flex from selection", line.split(":", 1)[1][:-1]
                        DoFlexFromWhat.set(2)
                        ListFlexResiduesNames.set(line.split(":", 1)[1][:-1])
                        ParseFlexSelection()
                        break
        except:
            tkMessageBox.showwarning("Flexible residues",
                                     ("There is a problem in reading the flexible residues information."))
    ...
    if LoadMap.get():
        ...
        if DEBUG: print
        "LOAD_SESSION> the grid mode is |%s|" % mode
        ...
        if "use pre-calculated" in mode:
            if DEBUG: print
            "    applying map mode 2 ( deferring map check)"
        ...
        if defer_map_check:
            ...
            for line in logfile:
                if "Grid cache dir" in line:
                    ...
                    if DEBUG:
                        print
                        "    (Deferred map checking)    found cache dir : |%s|" % cache_dir
                        print
                        "    List of map files:"
                        print
                        glob.glob(os.path.join(cache_dir, "*map*"))
...
def SaveLogWindow():
    ...
    keepasking = True
    while keepasking:
        logname = asksaveasfilename(parent=root, title="Select the Raccoon log filename to save...",
                                    filetypes=[('Raccoon log file', '*.log'), ("Any file...", "*")],
                                    defaultextension=".log")
        if logname:
            if DEBUG: print
            "here I should save the file log..."
    ...
    Label(SaveOptWin, text="Select data to be saved in the file:", justify=LEFT).grid(row=1, column=0, columnspan=3,
    ...
    Checkbutton(SaveOptWin, text="Flexible residues", variable=SaveFlex).grid(row=5, column=1, columnspan=2, sticky=W,
                                                                              padx=15)
    ...
    Button(SaveOptWin, text="Cancel", command=lambda: SaveOptWin.destroy()).grid(row=9, column=2, sticky=W, padx=10,
                                                                                 pady=10)
    EnableInterface()
...
def SaveLog(file):
    ...
    try:
        ...
        if DEBUG: print
        "SaveLog> problems saving session file %s [ %s ] " % (file, err.getwhatever)
        return False
                ...
                try:
                    if os.path.exists(DirName):
                        if len(glob.glob(os.path.join(DirName, "*"))):
                            ...
                            #    dir_accepted = True
                            tkMessageBox.showerror("Output directory",
                                                   'The selected directory contains some files but an empty directory is required to generate the VS jobs:\n\n - specify another directory\n - type a new name to create one.')
                        else:
                            dir_accepted = True
                    else:
                        if tkMessageBox.askokcancel('Output directory',
...
def CheckDiskSpace(path):
    ...
    if system == "Windows":  # Warning: cover your eyes 'cause this workaround is extremely ugly (...but it works)
        ...
        if DEBUG: print
        "CHECKDISKSPACE> this is path", path
        ...
        try:
            ...
            if DEBUG:
                print
                "CHECKDISKSPACE [%s] > there was an issue with the disk space" % system
                print
                "CHECKDISKSPACE [%s] > available = %s" % (system, available)
    ...
    if DEBUG: print
    "CHECKDISKSPACE> System = %s, Diskspace = %12.3f " % (system, available)
    ...
    if DEBUG: print
    "Calculated disk space: ", calculated_space
...
def countLigands():
    ...
    "countLigands> this would be the dictionary lenght:", len(LigandDictionary)
    ...
    if ligand_conscription > 1500:
        ...
        if DEBUG: print
        "countLigands> sub-splitting set to :", sub_splitting.get()
    return
...
def makeDir(path, name="out_dir"):
    ...
    try:
        if not os.path.exists(out_dir):
            ...
            if DEBUG: print
            "makeDir> the directory \"%s\" doesn't exist, creating" % out_dir
        else:
            if DEBUG: print
            "makeDir> the directory \"%s\" exist, passing..." % out_dir
        return out_dir
    except:
        if DEBUG: print
        "makeDir> Error in creating the directory : ", out_dir
        return False
...
def openLigand(ligFile=None, quiet=False):  # Now unified loader for all the supported formats
    ...
    problematic_list = []
    if not ligFile:
        ligFile = askopenfilename(parent=root, title="Select one or more PDBQT, PDB or (multi)MOL2",
                                  filetypes=[("Supported ligand formats", ("*.pdbqt", "*.mol2", "*.pdb")),
                                             ("PDBQT", "*.pdbqt"), ("PDB", "*.pdb"), ("Mol2", "*.mol2"),
                                             ("Any file type...", "*")], multiple=1)
    if ligFile:
        ...
        for file in ligFile:
            ...
            if DEBUG:
                print
                "openLigand> found files:"
                print
                "\t %d pdb" % len(pdb_list)
                print
                "\t %d pdbqt" % len(pdb_list)
                print
                "\t %d mol2" % len(pdb_list)
        ...
        for filename in pdb_list:
            ...
            if pdbqt_dir:
                ...
                problematic_list.append(
                    [filename, ("problem in generating PDBQT (dir creation: %s)" % dirname + os.sep + "pdbqt")])
...
def SplitMol2Interface(reset=False, import_result=False, input_filename=None):
    ...
    def open_a_mol2(filename=None):
        ...
        if filename:
            ...
            if not total:
                ...
                if total == 1:
                    tkMessageBox.showwarning("Warning!", (
                        "The selected file contains only a structure, therefore will not be splitted"))
                ...
                if DEBUG: print
                "SplitMol2Interface> open_a_mol2> Default dir = ", default_dir
                set_dir(default_dir)
    ...
    def set_dir(dir=None):
        ...
        if not dir:
            try:
                ...
                default_dir = ""
            dir = askdirectory(parent=Mol2OptionsWin, title=("MOL2: Split and convert in the following dir..."),
    ...
    def start_split():
        ...
        if DEBUG:
            print
            "Mol2Options> Triggering mol2 splitting with the following params:"
            print
            molfilename.get(), output_dir.get(), chunk[split]

        if SplitMol2(filename=molfilename.get(), rootdir=output_dir.get(), naming=mol2_naming_scheme.get(),
                     sub_splitting=chunk[split]):
            print
            "SplitMol2Interface> start_split> ImportedSplitted.get()", ImportSplitted.get()
            print
            "                                 output_dir.get()", output_dir.get()
    ...
    try:
        ...
        Button(InputFrame.interior(), compound=LEFT, text="Open...", image=open_smfolder_icon,
               command=open_a_mol2).grid(row=0, column=0, padx=5, sticky=W)
        ...
        Button(OutputFrame.interior(), compound=LEFT, text="Set...", image=open_smfolder_icon, command=set_dir).grid(
            row=0, column=0, padx=5, sticky=W, pady=3)
        ...
        SplitButton = Button(Mol2OptionsWin, compound=LEFT, text="Split", image=split_icon,
                             command=lambda: start_split(), state="disabled", width=120)
        # SplitButton.configure(status = "disabled")
        SplitButton.grid(row=0, column=0, padx=10, sticky=E + W + S, pady=10)
        Button(Mol2OptionsWin, compound=LEFT, text="Exit", command=lambda: Mol2OptionsWin.destroy()).grid(row=5,
                                                                                                          column=0,
                                                                                                          padx=10,
                                                                                                          pady=10,
                                                                                                          sticky=E + S)
...
def SplitMol2(filename=None, rootdir=None, naming="ZINC id", sub_splitting=1000):
    ...
    if DEBUG: print
    "SplitMol2> called with the following parameters:", filename, rootdir, naming, sub_splitting
    ...
    if sub_splitting:
        if sub_splitting >= total:
            sub_splitting = None
            if DEBUG: print
            "SplitMol2> no splitting is actually required"
        else:
            merge_remainder = False
            ...
            if DEBUG: print
            "SplitMol2> last dir would contain %d files =  %2.3f percent " % ((total % sub_splitting), remainder_pc)
            if remainder_pc < 10.0:
                merge_remainder = True
            if DEBUG: print
            "SplitMol2> merge_remainder = ", merge_remainder
            ...
            dir_format = "%0*d"
            if DEBUG: print
            "SplitMol2> dir_format is ", dir_format
    ...
    if not rootdir:
        tkMessageBox.showinfo(title="MOL2 splitting",
                              message=("The splitting process of %s has been cancelled by the user" % filename))
        return
    # end of utility mode
    ...
    try:
        if not os.path.exists(rootdir):
            if DEBUG: print
            "SplitMol2> the rootdir %s doesn't exists, therefore will be created" % rootdir
        ...
        if sub_splitting:
            ...
            if DEBUG: print
            "SplitMol2> initialized the first sub-directory : ", outdir
            if DEBUG: print
            "SplitMol2> (dir_count = %s ) ", dir_count

            if not os.path.exists(outdir):
                try:
                    if DEBUG: print
                    "SplitMol2> creating the new directory : ", outdir
                    os.makedirs(outdir, 0755)
                except:
                    tkMessageBox.showerror("Error!", (
                                "Impossible to create the following directory:\n%s\nThe splitting process is aborted." % outdir))
                    return False
        else:
            outdir = rootdir
                ...
                if len(buffer) > 0:  # INCREASED FROM 0
                    ...
                    if sub_splitting:  #
                        if (split_count % sub_splitting) == 0:  # or split_count < 1:
                            print
                            "%d == %d : %d" % (split_count, sub_splitting, (split_count % sub_splitting))
                            dir_count += 1
                            dir_name = dir_format % (size, dir_count)
                            outdir = rootdir + os.sep + dir_name
                            if DEBUG: print
                            "SplitMol2> the current directory is : ", outdir, "##########################################"
                            if not os.path.exists(outdir):
                                try:
                                    if DEBUG: print
                                    "SplitMol2> creating the new directory : ", outdir
        ...
        if utility_mode:
            tkMessageBox.showinfo(title="MOL2 splitting",
                                  message=("%d molecules have been successfully created in:\n %s" % (count, rootdir)))
        InfoMessage.set(("Splitting %s : DONE ") % os.path.basename(filename))
        root.update()
        return splitted_mols
    except:
        tkMessageBox.showerror("Error!", (
                    "The selected directory is not accessible: %s\nThe splitting process is aborted." % rootdir))
        return False
    if DEBUG: print
    count, "molecules processed"
...
def genPDBQT(infile, outfile):
    ...
    if DEBUG: print
    "genPDBQT> allowed bonds:", allowed_bonds
    # -r  root
    root = 'auto'
    # -o outputfilename
    ...
    if DEBUG: print
    "genPDBQT> Selected allowed bonds", SELECTED_ALLOWED_BONDS
...
def dist(f, s, sq=True):
    ...
    if sq:
        ...
        return (float(f[30:38]) - float(s[30:38])) ** 2 + \
            (float(f[38:46]) - float(s[38:46])) ** 2 + \
            (float(f[46:54]) - float(s[46:54])) ** 2
...
def openLigandDir(ligDir=None, recursive=False, do_confirm=True):
    ...
    def ligsummary():
        ...
        if count_pdb:
            ...
            Checkbutton(sumwin, text=("PDB\t [ %d ]" % count_pdb), variable=importPDBcheck, onvalue=True,
                        offvalue=False).grid(row=2, column=0, sticky=W, padx=10, pady=0)
        if count_mol2:
            importMol2check.set(True)
            Checkbutton(sumwin, text=("Mol2\t [ %d ]" % count_mol2), variable=importMol2check).grid(row=3, column=0,
                                                                                                    sticky=W, padx=10,
                                                                                                    pady=0)
    ...
    if ligDir:
        ...
        if DEBUG:
            print
            "This would be the ligand file list"
            print
            "pdbqt", pdbqt_ligandlist
            print
            "mol2", mol2_ligandlist
            print
            "pdb", pdb_ligandlist
...
def checkPDBQTligList(filenamelist, quiet=False):
    ...
    if CountAccepted == 0:
        if CountRejected == 1:
            ...
            tkMessageBox.showerror("Ligands error!",
                                   "None of %d PDBQT have been accepted.\nPlease check the inputs." % CountRejected)
    if not quiet:
        if CountAccepted > 0 and CountRejected > 0:
            if tkMessageBox.askokcancel("Ligands imported",
                                        "%d\taccepted.\n%d\trejected.\n\nDo you want to inspect the list of rejected ligands?" % (
    ...
    for file in filenamelist:
        ...
        try:
            ...
            if found_ligand == 1:
                ...
                if DEBUG: print
                file, "is bad receptor.."
            else:
                AcceptedFiles.append(file)
                CountAccepted += 1
                if DEBUG: print
                file, "a good receptor.."
        except:
            CountRejected += 1
            problematic_list.append([file, "error in opening the file"])
    if not quiet:
        ...
        if CountAccepted == 0:
            if CountRejected == 1:
                ...
                tkMessageBox.showwarning("Receptors error!",
                                         "None of %d receptors have been accepted.\nPlease check the inputs." % CountRejected)
        if CountAccepted > 0 and CountRejected > 0:
            if tkMessageBox.askokcancel("Receptors imported",
                                        "%d\taccepted.\n%d\trejected.\n\nDo you want to inspect the list of rejected structures?" % (
                                        CountAccepted, CountRejected)):
                issueInspectorWindow("List of rejected PDBQT's", problematic_list)
    return AcceptedFiles, problematic_list
...
def issueInspectorWindow(title, issue_list):
    ...
    def saveList():
        file = askopenfilename(title="Select error list log filename...",
                               filetypes=[("CSV error file", "*.csv"), ("Any file...", "*")])
        if not file:
            return
        try:
            fp = open(file, 'wb')
...
def CheckLigFilterOptions():
    for value in HbDmin, HbDmax, HbAmin, HbAmax, MWmin, MWmax, NatMin, NatMax, TORSDOFmin, TORSDOFmax:
        try:
            ...
            break
        if value.get() < 0:
            tkMessageBox.showwarning("Uhmmmm...",
    ...
    if TORSDOFmax.get() + FlexResTORSDOF.get() > AutoDockMaxTORSDOF.get():
        if DEBUG: print
        TORSDOFmax.get(), FlexResTORSDOF.get(), AutoDockMaxTORSDOF.get()
        if FlexResTORSDOF.get() > 0:
            tkMessageBox.showwarning("Torsion Limit",
                                     "WARNING: AutoDock can handle up to %s rotatable bonds.\n\nThere are %s rotatable bonds already allotted for the flex residues, therefore the maximum number of rotatable bonds per ligand should be %s.\n\nUse this value carefully." % (
                                     AutoDockMaxTORSDOF.get(), FlexResTORSDOF.get(),
                                     AutoDockMaxTORSDOF.get() - FlexResTORSDOF.get()))
        else:
            tkMessageBox.showwarning("Torsion Limit",
                                     "WARNING: AutoDock can handle up to %s rotatable bonds. Use this value carefully..." % (
                                         AutoDockMaxTORSDOF.get()))
...
def LigandImportOptions():  # Options for non-PDBQT files only
    ...
    try:
        ...
        Radiobutton(ChargeFrame.interior(), text="add Gasteiger", variable=ChargeSet, value="gasteiger",
                    highlightbackground='black', indicatoron=0, selectcolor='lightblue', width=23, height=2).grid(row=0,
                                                                                                                  column=0,
                                                                                                                  sticky=W,
                                                                                                                  padx=2,
                                                                                                                  pady=3)  # Default
        ...
        Radiobutton(RepairFrame.interior(), text="[ off ]", variable=Repair, value="",
                    highlightbackground='black', indicatoron=0, selectcolor='lightblue', width=11).grid(row=1, column=0,
                                                                                                        sticky=N,
                                                                                                        padx=1,
                                                                                                        pady=5)  # Default
        Radiobutton(RepairFrame.interior(), text="bonds", variable=Repair, value="bonds",
                    highlightbackground='black', indicatoron=0, selectcolor='lightblue', width=11).grid(row=1, column=1,
                                                                                                        sticky=N,
                                                                                                        padx=1, pady=5)
        Radiobutton(RepairFrame.interior(), text="hydrogens", variable=Repair, value="hydrogens",
                    highlightbackground='black', indicatoron=0, selectcolor='lightblue', width=11).grid(row=1, column=2,
                                                                                                        sticky=N,
                                                                                                        padx=1, pady=5)
        Radiobutton(RepairFrame.interior(), text="both", variable=Repair, value="bond_hydrogens",
                    highlightbackground='black', indicatoron=0, selectcolor='lightblue', width=11).grid(row=1, column=3,
                                                                                                        sticky=N,
                                                                                                        padx=1, pady=5)
        ...
        Radiobutton(CleanupFrame.interior(), text="[ off ]", variable=Cleanup, value="",
                    highlightbackground='black', indicatoron=0, selectcolor='lightblue', width=11, height=2).grid(row=1,
                                                                                                                  column=0,
                                                                                                                  sticky=N,
                                                                                                                  padx=1,
                                                                                                                  pady=5)
        ...
        Radiobutton(RotatableFrame.interior(), variable=BackboneRotatable, value=True, selectcolor='green').grid(row=1,
                                                                                                                 column=1,
                                                                                                                 sticky=W)  # Default
        Radiobutton(RotatableFrame.interior(), variable=BackboneRotatable, value=False, selectcolor='red').grid(row=1,
                                                                                                                column=2,
                                                                                                                sticky=W)
        Label(RotatableFrame.interior(), text="     amide   ").grid(row=2, column=0, sticky=W)
        Radiobutton(RotatableFrame.interior(), variable=AmideRotatable, value=True, selectcolor='green').grid(row=2,
                                                                                                              column=1,
                                                                                                              sticky=W)  # Raccoon default
        Radiobutton(RotatableFrame.interior(), variable=AmideRotatable, value=False, selectcolor='red').grid(row=2,
                                                                                                             column=2,
                                                                                                             sticky=W)  # ADT default
        Label(RotatableFrame.interior(), text=" guanidinium   ").grid(row=3, column=0, sticky=W)
        Radiobutton(RotatableFrame.interior(), variable=GuanidiniumRotatable, value=True, selectcolor='green').grid(
            row=3, column=1, sticky=W)
        Radiobutton(RotatableFrame.interior(), variable=GuanidiniumRotatable, value=False, selectcolor='red').grid(
            row=3, column=2, sticky=W)  # Default
        ...
        Checkbutton(LigandOptionsWin, text="Inactivate ALL active\ntorsions", variable=LockTors,
                    indicatoron=False, highlightbackground='black', selectcolor='lightblue').grid(row=5, column=1,
                                                                                                  sticky=W, rowspan=1,
                                                                                                  padx=5, pady=5,
                                                                                                  ipadx=3, ipady=3)
        ...
        Radiobutton(FragmentFrame.interior(), text="keep all", variable=LargestFrag, value=False,
                    selectcolor='lightblue').grid(row=0, column=2, sticky=W, )  # Default
        Checkbutton(FragmentFrame.interior(), text="Attach non-bonded fragments", variable=AttachFrag,
                    highlightcolor='black', selectcolor='lightblue', indicatoron=False).grid(row=1, column=0,
                                                                                             sticky=W + E, columnspan=4,
                                                                                             ipadx=2, ipady=2)
        ...
        Tkinter.Button(LigandOptionsWin, compound=LEFT, text="OK", image=ok_icon, command=LigandOptionsWin.destroy,
                       highlightbackground='black').grid(row=9, column=0, sticky=S + W + E, columnspan=2, pady=10,
                                                         padx=10)
        Tkinter.Button(LigandOptionsWin, compound=LEFT, text="Set defaults", image=default_setting_icon,
                       command=DefaultLigOptions,
                       highlightbackground='black').grid(row=1, column=0, columnspan=2, sticky=S + W + E, pady=10,
                                                         padx=10)
...
def LigandFilterOptions():  # Filtering files
    ...
    try:
        ...
        Label(PresetFrame, text="Filter presets :").grid(row=0, column=0, columnspan=1, sticky=E,
                                                         padx=10)  # , padx = 30)
        PrecannedFilters = OptionMenu(PresetFrame, FilterSet, "Default", "Lipinski-like", "DrugLikeness",
                                      "DrugLikeness (frag)",
        ...
        HbDonors1 = Entry(FiltersFrame.interior(), width=4, textvariable=HbDmin).grid(row=2, column=1, sticky=W,
                                                                                      columnspan=1)
        HbDonors2 = Entry(FiltersFrame.interior(), width=4, textvariable=HbDmax).grid(row=2, column=2, sticky=W,
                                                                                      columnspan=1)
        ...
        HbAcceptors1 = Entry(FiltersFrame.interior(), width=4, textvariable=HbAmin).grid(row=3, column=1, sticky=W,
                                                                                         columnspan=1)
        HbAcceptors2 = Entry(FiltersFrame.interior(), width=4, textvariable=HbAmax).grid(row=3, column=2, sticky=W,
                                                                                         columnspan=1)
        Button(FiltersFrame.interior(), text="Default", image=default_setting_icon, highlightbackground='black',
        ...
        HbAcceptors1 = Entry(FiltersFrame.interior(), width=4, textvariable=MWmin).grid(row=4, column=1, sticky=W,
                                                                                        columnspan=1)
        HbAcceptors2 = Entry(FiltersFrame.interior(), width=4, textvariable=MWmax).grid(row=4, column=2, sticky=W,
                                                                                        columnspan=1)
        Button(FiltersFrame.interior(), text="Default", image=default_setting_icon, highlightbackground='black',
        ...
        HbAcceptors1 = Entry(FiltersFrame.interior(), width=4, textvariable=NatMin).grid(row=5, column=1, sticky=W,
                                                                                         columnspan=1)
        HbAcceptors2 = Entry(FiltersFrame.interior(), width=4, textvariable=NatMax).grid(row=5, column=2, sticky=W,
                                                                                         columnspan=1)
        Button(FiltersFrame.interior(), text="Default", image=default_setting_icon, highlightbackground='black',
        ...
        HbAcceptors1 = Entry(FiltersFrame.interior(), width=4, textvariable=TORSDOFmin).grid(row=6, column=1, sticky=W,
                                                                                             columnspan=1)
        HbAcceptors2 = Entry(FiltersFrame.interior(), width=4, textvariable=TORSDOFmax).grid(row=6, column=2, sticky=W,
                                                                                             columnspan=1)
        Button(FiltersFrame.interior(), text="Default", image=default_setting_icon, highlightbackground='black',
...
def FilteringDefaults(param=None):
    ...
    if verbose or DEBUG: print
    "FILTERING DEFAULTS> setting the max TORSDOF to ", max_tors
    ...
    if CheckLigFilterOptions():
        for lig in LigandDictionary.keys():
            ...
            if lig_notstdat and DoRejectATypes.get():
                ...
                if verbose or DEBUG: print
                lig, "rejected by NonSTDatom type", LigandDictionary[lig]["NotStdAT"]
            else:
                        ...
                        if lig_tors >= TORSDOFmin.get() and lig_tors <= TORSDOFmax.get():
                            if lig_mw >= MWmin.get() and lig_mw <= MWmax.get():
                                if lig_nat >= NatMin.get() and lig_nat <= NatMax.get():
                                    ...
                                    rejected_ligands.append(lig)
                                    if verbose or DEBUG: print
                                    lig, "rejected number of atoms", lig_nat
                            else:
                                rejected_ligands.append(lig)
                                if verbose or DEBUG: print
                                lig, "rejected molecular weight", lig_mw
                        else:
                            rejected_ligands.append(lig)
                            if verbose or DEBUG: print
                            lig, "rejected torsions", lig_tors
                    else:
                        rejected_ligands.append(lig)
                        if verbose or DEBUG: print
                        lig, "rejected HBD", lig_hbd
                else:
                    rejected_ligands.append(lig)
                    if verbose or DEBUG: print
                    lig, "rejected HBA", lig_hba
...
def LigandsTag():
    ...
    if DEBUG:
        print
        "Lenght of the LigandScrolledListBox.size(): ", LigandScrolledListBox.size()
    for item in range(0, LigandScrolledListBox.size()):
        if DEBUG: print
        "Tagging item ", item, LigandScrolledListBox.get(item)
...
def openReceptor(filename=None):
    ...
    if filename:
        ...
        recFile = askopenfilename(title="Select one or more target structures...",
                                  filetypes=[("Protein/DNA PDBQT", "*.pdbqt"), ("PDB", "*.pdb"), ("Mol2", "*.mol2")],
                                  multiple=1)
    if recFile:
        queue_in = []
        queue_out = []
...
def removeAllReceptors():
    ...
    if len(receptorFileList) > 1 and not tkMessageBox.askokcancel('Delete all the receptors.',
...
def openSingleReceptor(filename=None):
    if filename:
        ...
        SingleRecFileAsk = askopenfilename(
            filetypes=[("Supported formats", ("*.pdbqt", "*.mol2", "*.pdb")), ("PDBQT", "*.pdbqt"), ("PDB", "*.pdb"),
                       ("Mol2", "*.mol2")])
...
def genPDBQTrec(infile, outfile):
    ...
    if charges_to_add is not None and preserve_charge_types is not None:
        ...
        for t in preserved_types:
            if verbose: print
            'preserving charges on type->', t
            if not len(t): continue
            ats = mol.allAtoms.get(lambda x: x.autodock_element == t)
            if verbose: print
            "preserving charges on ", ats.name
            for a in ats:
    ...
    if len(mols) > 1:
        ...
        for m in mols[1:]:
            ...
            if len(m.allAtoms) > len(mol.allAtoms):
                mol = m
                if verbose: print
                "mol set to ", ctr, "th molecule with", len(mol.allAtoms), "atoms"
    mol.buildBondsByDistance()
    ...
    try:
        ...
        if DEBUG: print
        "genPDBQTrec> problems in generating receptor PDBQT", receptor_filename
        return False
...
def SetFlexibleResidueFile(get_name=None):
    ...
    if get_name:
        ...
        try:
            ...
            tkMessageBox.showerror("Flexibile Residue Error",
                                   "Error while opening the flexible residue file:\n" + get_name)
            return False
        ...
        if ResidueCount:
            ...
            ###
            ResidueStatusLoaded.set(
                str(ResidueCount) + " residue" + S + " loaded [ " + str(FlexResTORSDOF.get()) + " rotatable bonds ]")
...
def FlexTorsionCount(force=False):
    ...
    if DoFlexFromWhat.get() == 1 and not FlexResFileName.get() == "":
        try:
            ...
            if DEBUG: print
            "FlexTorsionCount> problems in reading the flex file:", FlexResFileName.get()
    ...
    if DoFlexFromWhat.get() == 2 and not ListFlexResiduesNames.get() == "":
        ...
        if verbose or DEBUG: print
        " adding %s rot's for %s" % (ResidueRotatableBondTable[res_nam][0], res_nam)
        if verbose or DEBUG: print
        "FLEX> this is the final count of rotatable bonds included for the flex res count:", found_flex_tors
...
value." % (found_flex_tors, AutoDockMaxTORSDOF.get() - found_flex_tors)))
    ...
    if DEBUG:
        print
        "FLEX> found a total of %s rotatable bonds in the flexible residue" % found_flex_tors
    return True
    ...
    if not selection == ['']:  # empty residue list... don't waste my time, please...
        for receptor in receptor_list:
            ...
            for res in set(selection):  # changed from "selection" to "set(selection)" to avoid duplicates
                ...
                res_nam = res[-1][0:3]
                if verbose or DEBUG: print
                "residue name  =>", res_nam
                res_num = res[-1][3:]
                if verbose or DEBUG: print
                "residue numb  =>", res_num
                if res_num == '':
                    if verbose or DEBUG: print
                    "NO NUMBER SPECIFIED!"
                        ...
                        if res_nam == line[17:20].split()[0]:
                            if res_num == line[22:26].split()[0]:
                                if chain in line[21]:
                                    FOUND = True
                                    if verbose or DEBUG: print
                                    line  # useful for debugging, do not remove!
                                    if line[21] not in chain_list:
                                        chain_list.append(line[21])
                if FOUND:
                    if verbose or DEBUG: print
                    "FOUND THE RESIDUE %s in %s" % (res, receptor)
                    found.append([residue, (len(chain_list))])
                    del chain_list[:]
                else:
                    missing.append(residue)
                    if verbose or DEBUG: print
                    "XXXX this res is missing: ", res
                    if verbose or DEBUG:    print
                    "this is a window with an error of residue"
            if len(missing) > 0:
                if verbose or DEBUG: print
                "XXXX some residues are missing"
                if verbose or DEBUG: print
                "this is a window with an error of residue"
                if verbose or DEBUG: print
                missing
...
are missing:\n" + msg_missing + "\n\n - Check the syntax (i.e. \'THR276', 'B:THR276' or 'B:THR276,B:HIS229')\n\n - Specify different residues.\n"))
        ...
        if len(selection) == len(found):
            ...
            if not FlexTorsionCount():
                return False
            ResidueStatusSelected.set(
                (str(total) + " residue(s) selected [ " + str(FlexResTORSDOF.get()) + " rotatable bonds ]"))
...
def MakeReceptorMenu():
    ...
    Single_target_radio = Radiobutton(ReceptorRadioChoice, compound=RIGHT, text='Single\ntarget', width=120,
                                      image=single_rec_icon, value=0, variable=RCstatus, state=ACTIVE,
                                      command=ReceptorOptions,
                                      indicatoron=False, selectcolor='lightblue', disabledforeground='gray',
    ...
    Multi_target_radio = Radiobutton(ReceptorRadioChoice, compound=LEFT, text='Multiple\ntargets', width=120,
                                     image=multi_rec_icon, value=1, variable=RCstatus, command=ReceptorOptions,
                                     indicatoron=False,
    ...
    SingleRecButton = Button(SingleTargetPanel, compound=LEFT, text="Add receptor file...", image=add_single_icon,
                             command=openSingleReceptor)
    ...
    AddAReceptorButton = Button(MultiTargetPanel, compound=LEFT, text='Add a structure...', image=add_single_icon,
                                command=openReceptor)
    ...
    AddDirRecButton = Button(MultiTargetPanel, compound=LEFT, text='Add a directory...', image=add_dir_icon,
                             command=openReceptorDir)
    ...
    DelSelRec = Button(MultiTargetPanel, compound=LEFT, text='Remove selected', image=remove_selected_icon,
                       command=removeReceptor)
    ...
    DelAllRec = Button(MultiTargetPanel, compound=LEFT, text='Remove all', image=remove_all_icon,
    ...
    group_receptor2 = Pmw.Group(p2, tag_text='Flexible residues', tag_pyclass=Tkinter.Checkbutton, tag_variable=DoFlex,
                                tag_command=SetFlexibleMode)
    group_receptor2.pack(expand=0, padx=5, pady=3, anchor=N, side=TOP)
    group_receptor2.bind("<Button-3>", lambda x: help("flexres"))
    FlexResFile = Radiobutton(group_receptor2.interior(), text='From file', value=1, variable=DoFlexFromWhat,
                              command=EnableFlexFromFile)
    ...
    FlexResList = Radiobutton(group_receptor2.interior(), text='From selection', value=2, variable=DoFlexFromWhat,
                              command=EnableFlexFromSel)
    ...
    DefaultRec = Tkinter.Button(p2, compound=LEFT, text="Target PDBQT generation", image=pdbqt_lig_opt_icon,
                                command=ReceptorImportOptions)
...
def ReceptorImportOptions():
    ...
    try:
        ...
        Radiobutton(ChargeFrame.interior(), text="Add Gasteiger", variable=RecChargeSet, value="gasteiger",
                    indicatoron=0, width=15,
                    highlightbackground='black', selectcolor='lightblue').grid(row=0, column=0, sticky=W, padx=5,
                                                                               pady=5)  # Default
        Radiobutton(ChargeFrame.interior(), text="Keep original", variable=RecChargeSet, value="", indicatoron=0,
                    width=15,
                    highlightbackground='black', selectcolor='lightblue').grid(row=0, column=1, sticky=W, padx=5,
                                                                               pady=5)
        ...
        Radiobutton(CleanupFrame.interior(), variable=RecCleanNPH, value="_nphs", selectcolor="green").grid(row=1,
                                                                                                            column=1,
                                                                                                            sticky=N,
                                                                                                            pady=3)
        Radiobutton(CleanupFrame.interior(), variable=RecCleanNPH, value="", selectcolor="red").grid(row=1, column=2,
                                                                                                     sticky=N, pady=3)
        Label(CleanupFrame.interior(), text="Lone pairs").grid(row=2, column=0, sticky=E, pady=3)
        Radiobutton(CleanupFrame.interior(), variable=RecCleanLP, value="_lps", selectcolor='green').grid(row=2,
                                                                                                          column=1,
                                                                                                          sticky=N,
                                                                                                          pady=3)
        Radiobutton(CleanupFrame.interior(), variable=RecCleanLP, value="", selectcolor='red').grid(row=2, column=2,
                                                                                                    sticky=N, pady=3)
        Label(CleanupFrame.interior(), text="Water molecules").grid(row=3, column=0, sticky=E, pady=3)
        Radiobutton(CleanupFrame.interior(), variable=RecCleanWAT, value="_waters", selectcolor='green').grid(row=3,
                                                                                                              column=1,
                                                                                                              sticky=N,
                                                                                                              pady=3)
        Radiobutton(CleanupFrame.interior(), variable=RecCleanWAT, value="", selectcolor='red').grid(row=3, column=2,
                                                                                                     sticky=N, pady=3)
        Label(CleanupFrame.interior(), text="Non-standard residues").grid(row=4, column=0, sticky=E, pady=3)
        Radiobutton(CleanupFrame.interior(), variable=RecCleanStdRes, value=True, selectcolor='green').grid(row=4,
                                                                                                            column=1,
                                                                                                            sticky=N,
                                                                                                            pady=3)
        Radiobutton(CleanupFrame.interior(), variable=RecCleanStdRes, value=False, selectcolor='red').grid(row=4,
                                                                                                           column=2,
                                                                                                           sticky=N,
                                                                                                           pady=3)
        Label(CleanupFrame.interior(), text="Alternate conformations").grid(row=5, column=0, sticky=E, pady=3)
        Radiobutton(CleanupFrame.interior(), variable=RecDelAlternate, value="deleteAltB", selectcolor='green').grid(
            row=5, column=1, sticky=N, pady=3)
        Radiobutton(CleanupFrame.interior(), variable=RecDelAlternate, value="", selectcolor='red').grid(row=5,
                                                                                                         column=2,
                                                                                                         sticky=N,
                                                                                                         pady=3)
        ...
        # label
        RecRepairOptions = OptionMenu(RepairFrame.interior(), RecRepairOptionsSet, "none", "rebuild bonds", "add H",
                                      "add H (if missing)",
                                      "rebuild bonds + add H")
        ...
        Tkinter.Button(ReceptorOptionsWin, compound=LEFT, text="Set defaults", image=default_setting_icon,
                       command=ReceptorOptionsDefault,
...
def ReceptorOptions():
    ...
    if RCstatus.get() == 0:  # Single Target
        if verbose: print
        "I'm running the RCstatus with this value:", RCstatus.get()
...
def MakeGPFMenu():
    ...
    CacheMapOptionsNow = OptionMenu(CacheMapHandleNow.interior(), CacheMapPolicy, "Make copies [ use more disk space ]",
                                    "Make symbolic links [ save disk space ]")
    CacheMapPolicy.set('Make copies [ use more disk space ]')
    ...
    CacheMapDir = Button(CacheMapFrame.interior(), compound=LEFT, text='Select cached maps directory',
                         image=open_smfolder_icon, command=opendirMaps)  # , state = DISABLED)
    ...
    CacheMapOptions = OptionMenu(CacheMapHandle.interior(), CacheMapPolicy, "Make copies [ use more disk space ]",
                                 "Make symbolic links [ save disk space ]")
    CacheMapPolicy.set('Make copies [ use more disk space ]')
    ...
    if gpfFile:
        ...
        for line in get_lines(gpfFile):  # surprisingly, adding the text to the editor worked at the first attempt....
            if line[0:14] == "parameter_file":
                if DEBUG: print
                "I found a param file in %s!" % gpfFile
        ...
        if ask_for_param_file == 1:
            tkMessageBox.showwarning("Parameter file required",
                                     "A parameter file is required by the GPF:\n => %s \n The filename location must be specified..." % param_file_name)
...
def CheckFolderMap(MapFileList):
    ...
    if DEBUG: print
    "CheckFolderMap> RECEPTOR_STEM", receptor_stem
    # Check for all atom types maps
    name_consistency = True
    if DEBUG:
        for x in AtypeList: print
        x, AtypeList[x][0]
...
name.\n\n\t'%s'\n\n Are you sure you want to continue?" % (receptor_stem + ".pdbqt"))):
    ...
    if len(MissingMaps) == 0:
        ...
        try:
            ...
            if DEBUG: print
            "CheckFolderMap> Houston, we've got a problem in checking maps..."
            MapConsistency = False
...
def opendpf():
    ...
    if dpfFile:
        ...
        if ask_for_param_file == 1:
            tkMessageBox.showwarning("Parameter file required",
                                     "A parameter file is required by the DPF:\n => %s \n The filename location must be specified..." % param_file_name)
...
def get_AD_atom_types(filename):
    ...
    try:
        ...
        if DEBUG: print
        "get_AD_atom_types> problems in reading the file: ", filename
...
def prepareGPFfast(gpf_filename, receptor_filename, rec_types, lig_types):
    ...
    if not rec_types:
        tkMessageBox.showerror("Receptor properties error",
                               ("Impossible to access to receptor properties, aborting..."))
        if DEBUG: print
        "prepareDPFfast> problems in reading the file: ", ligand_filename
        return False
    ...
    try:
        ...
        if DEBUG: print
        "prepareGPFfast> problems in saving the file: ", gpf_filename
        tkMessageBox.showerror("GPF generation error", ("Impossible to save the file\n" + gpf_filename + "\n\n"))
        return False


def prepareDPFfast(dpf_filename, receptor_filename, ligand_filename, lig_types, flex_res_filename=None,
                   parameter_file=None):
    # TODO
    ...
    def GetParms(dpf_parms):
        ...
        ga_run = "10"
        clustering = ""
        for i, l in enumerate(dpf_parms):
            l = l.split("#")[0]
            l = l.split()
            if DEBUG: print
            "prepareDPFfast> line parsing:", l
            if l:
                if "outlev" in l:
                    outlev = l[1]
                ...
                if 'intnbp_r_eps' in l:
                    intnbp_r_eps.append(dpf_parms[i])
                ...
                if "analysis" in l:
                    clustering = "analysis"
        return (
            outlev, ga_pop_size, ga_num_evals, intnbp_r_eps, ga_num_generations,
            ga_elitism, ga_mutation_rate, ga_crossover_rate, ga_window_size,
            ga_cauchy_alpha, ga_cauchy_beta, sw_max_succ, sw_max_its, sw_max_fail,
            sw_rho, sw_lb_rho, ls_search_freq, local_search_mode, rms_tol, energy_model, ga_run,
            clustering
        )
    ...
    (outlev, ga_pop_size, ga_num_evals, intnbp_r_eps, ga_num_generations,
     ...
     ga_cauchy_alpha, ga_cauchy_beta, sw_max_succ, sw_max_its, sw_max_fail,
     sw_rho, sw_lb_rho, ls_search_freq, local_search_mode, rms_tol, energy_model, ga_run,
     clustering) = GetParms(dpf_lines)

    # ligand data
    try:
        about = get_mol_center(ligand_filename)
        if DEBUG: print
        "prepareDPFfast> LIGAND: %s\t\t about : %3.2f  %3.3f %3.3f" % (ligand_filename, about[0], about[1], about[2])
        if not lig_types:
        ...
        if DEBUG: print
        "prepareDPFfast> problems in getting ligand center: ", ligand_filename
        tkMessageBox.showerror("Receptor properties error",
                               ("Impossible to access to receptor properties, aborting..."))
    ...
    if DEBUG: print
    "prepareDPFfast> getting flex_res atom types"
    if flex_res_filename:
    ...
    try:
        ...
        if DEBUG: print
        "prepareDPFfast> problems in saving the file: ", dpf_filename
...
def GetOSoption():
    ...
    if not TargetOS.get():
        # TODO clarify this
        if DEBUG: print
        "this is the first time the TargetOS is called"
        TargetOS.set('lin')
    ...
    if TargetOS.get() == "lin":
        ...
        Checkbutton(panel.interior(), text="delete map files after docking", var=RemoveMapsAfter).grid(row=5, column=1,
                                                                                                       columnspan=2,
                                                                                                       sticky=W)
    ...
    if TargetOS.get() == "pbs":
        ...
        CheckMakeTarGZ = Tkinter.Label(panel.interior(), text='Create a VS package file ').grid(row=2, column=0,
        ...
        PBSOptionCPUset = Button(panel.interior(), text="Set", command=SetPBShowmanyruns).grid(row=4, column=2,
                                                                                               sticky=W)
        SetPBStime
        SetPBShowmanyruns
        ...
        Checkbutton(panel.interior(), text="delete map files after docking", var=RemoveMapsAfter).grid(row=6, column=1,
                                                                                                       columnspan=2,
                                                                                                       sticky=W)
...
def SetPBShowmanyruns():
    ...
    if wrong:
        ...
        tkMessageBox.showerror("PBS runs error!", (
            "The value of runs must be a number bigger than 0 and smaller than infinite.\n\nReset to default."))
        PBShowmanyruns.set(1)
        return False
    else:
        if howmany > 255:
...
def SetPBStime():
    if DEBUG: print
    "CHECKING TIME FOR PBS", PBStime.get()
    time = PBStime.get()
    try:
        time = time.split(':')
    except:
...
def TheCheck(parameter=None):
    ...
    if DEBUG: print
    "- ligands", TotalAcceptedLigands.get()
    ...
    if RECEPTORS:
        if DoFlex.get():
            ...
            if FLEXIBLE:
                if DEBUG: print
                "THE_CHECK> we're going flexible..."
...
def TheFunction():
    ...
    if DEBUG: print
    "TheFunction> starting the vs creation in ", path
    ...
    if not log_file:
        if DEBUG: print
        "[gen+log] => we start very well... :S no logging available!"
        EnableInterface()
        return False
    ...
    if RCstatus.get() == 0:
        if DEBUG: print
        "[gen] => single receptor NAME = ",
        receptor_list = [RecFilename.get()]
        if DEBUG: print
        RecFilename.get()
    else:
        if DEBUG: print
        "[gen] => multiple receptors",
        multi_receptor_paths_list = []
        receptor_list = receptorScrolledListBox.get('0', END)

    if DEBUG: print
    "[ I'm going to use %d receptors ]" % len(receptor_list)
    ...
    for receptor in receptor_list:
        ...
        if not os.path.exists(current_path):
            try:
                ...
                tkMessageBox.showerror("Receptor dir error!", (
                        "Impossible to create the directory:\n%s\n GIVING UP...\n(Error:%s)" % (
                    current_path, sys.exc_info()[1])))
        ...
        if DoFlex.get():  #
            if FlexResDefined.get():  #
                if DEBUG: print
                "\tsoo.... we want flexible,right?\n\nPREPARING"
                if DoFlexFromWhat.get() == 1:
                    if FlexResFileName.get():  # TODO in theory it shouldn't be necessary
                        if DEBUG: print
                        "\tcopying the flexible residue in the right place"
                        if DEBUG: print
                        "\tcp flex.pdbqt working_dir"
                        flex_res = FlexResFileName.get()
                if DoFlexFromWhat.get() == 2:
                    if FlexResSelected.get():  # TODO in theory it shouldn't be necessary
                        if DEBUG: print
                        "\tgenerate the flexible residue from the receptor"
                        if DEBUG: print
                        "\tprepare_flex_receptor blablabla "
                        InfoMessage.set(("[ Generating flex residues for %s... ]" % rec_name))
                        receptor, flex_res = genFlex(receptor)
                        if not flex_res:
                            if DEBUG: print
                            "TheFunction> generation aborted"
                            tkMessageBox.showerror("Error!", (
                                        "Problems generating the flexible residues:\n receptor: %s\n\n\nGeneration aborted." % (
                                    receptor)))
                            return False
        ...
        if DEBUG:
            print
            "\t[GEN] I've got the flex_res filename", flex_res
            print
            "\t[GEN] Now the receptor is", receptor
        #
        ...
        if MapSource.get() >= 1:
            ...
            if not os.path.exists(CachedMapsDir):
                try:
                    ...
                    tkMessageBox.showerror("Maps caching", (
                                "Impossible to create the directory:\n%s\n GIVING UP...\n(ERROR: %s)" % (
                        CachedMapsDir, sys.exc_info()[1])))
                    # print >> log_file, ("\n\n\n#### ERROR ###\n\nThere was a problem in creating the directory:\n%s\n\n VS generation aborted.\n\n####      ####" % CachedMapsDir)
                    log_file.write(
                        "\n\n\n#### ERROR ###\n\nThere was a problem in creating the directory:\n%s\n\n VS generation aborted.\n\n####      ####" % CachedMapsDir)
                    EnableInterface()
        ...
        if MapSource.get() == 2:  # populate the dir by copying the files from the cache
            ...
            if not CopyMapDir(atomtypes_set, CacheMapDirName.get(), CachedMapsDir, symlink=False):
                tkMessageBox.showerror("Error!", (
                            "Problems in copying the maps in the VS job master directory \n%s\n GIVING UP...\n(ERROR: %s)" % (
                    CachedMapsDir, sys.exc_info()[1])))
                EnableInterface()
                return False
        ...
        for ligand in ligand_list:
            ...
            InfoMessage.set(("=> Processing %s | %s \t[ %d | %d ]" % (
            os.path.basename(receptor), os.path.basename(ligand), jobs_done, jobs_todo)))
            root.update()

            ligand_name = os.path.basename(ligand).rsplit('.', 1)[:-1][0]
            if DEBUG: print
            "\tTheFunction> processing the ligand ", ligand_name
            # create ligand dir
            if not splitting:
                ...
                if DEBUG: print
                "\tTheFunction> splitting ligand dirs has been required"
                if counter == splitting or counter == 1e50:
                    ...
                    try:
                        ...
                        tkMessageBox.showerror("Ligand directory splitting", (
                                "Impossible to create the directory:\n%s\n GIVING UP...\n(ERROR: %s)" % (
                            output_dir, sys.exc_info()[1])))
                        # print >> log_file, ("\n\n\n#### ERROR ###\n\nThere was a problem in creating the directory:\n%s\n\n VS generation aborted.\n\n####      ####" % output_dir)
                        log_filewrite(
                            "\n\n\n#### ERROR ###\n\nThere was a problem in creating the directory:\n%s\n\n VS generation aborted.\n\n####      ####" % output_dir)
                        TheButton.config(state=DISABLED, text=" [ Generation aborted ]")
            ...
            if not ligand_dir:
                tkMessageBox.showerror("Ligand directory", (
                            "Impossible to create the directory:\n%s\n GIVING UP...\n(ERROR: %s)" % (
                    ligand_dir, sys.exc_info()[1])))
                # print >> log_file, ("\n\n\n#### ERROR ###\n\nThere was a problem in creating the directory:\n%s\n\n VS generation aborted.\n\n####      ####" % ligand_dir)
                log_file.write(
                    "\n\n\n#### ERROR ###\n\nThere was a problem in creating the directory:\n%s\n\n VS generation aborted.\n\n####      ####" % ligand_dir)
                TheButton.config(state=DISABLED, text=" [ Generation aborted ]")
            ...
            if not os.path.dirname(ligand) == ligand_dir:
                try:
                    ...
                    tkMessageBox.showerror("Error!", (
                                "Impossible to copy the ligand:\n%s\n\tto\n%s\n\nGIVING UP...\n(ERROR: %s)" % (
                        ligand, ligand_dir, sys.exc_info()[1])))
                    TheButton.config(state=DISABLED, text=" [ Generation aborted ]")
                    EnableInterface()
                    return False
            else:
                if DEBUG: print
                "TheFunction> skipped the source/dest ligand copy because they are identical..."
                ...
                if FlexResDefined.get():
                    if not os.path.dirname(flex_res) == ligand_dir:
                        try:
                            ...
                            tkMessageBox.showerror("Error!", (
                                        "Impossible to copy the flex res file:\n%s\n\tto\n%s\n\nGIVING UP...\n(ERROR: %s)" % (
                                flex_res, ligand_dir, sys.exc_info()[1])))
                            TheButton.config(state=DISABLED, text=" [ Generation aborted ]")
                            EnableInterface()
                            return False
                    else:
                        if DEBUG: print
                        "TheFunction> skipped the source/dest flex_res copy because they are identical..."
            ...
            if MapSource.get() == 0:  # being in the for loop cached maps will be referred to the receptor
                ...
                if not prepareGPFfast(gpf_file, receptor, rec_types=rec_atypes, lig_types=lig_atypes):
                    ...
                    # except:
                    tkMessageBox.showerror("PrepareGPF", (
                                "Error!\n\nImpossible to create the GPF file:\n%s\n\n     GIVING UP...\n(ERROR:%s)" % (
                        gpf_file, sys.exc_info()[1])))
                    TheButton.config(state=DISABLED, text=" [ Generation aborted ]")
                    EnableInterface()
                    return False
                ...
                if GPFParameterFile.get():
                    if DEBUG:
                        print
                        "|%s|<= This is the parameter file" % GPFParameterFile.get()
                        print
                        GPFParameterFile.get() == ""
                        print
                        GPFParameterFile.get() == None
 ...
 GPF:\nparameter file: %s\n\ndestination: %s\n\nGeneration aborted." % (GPFParameterFile.get(), ligand_dir)))
                        ...
                        if DEBUG: print
                        "TheFunction> skipped the source/dest GPFparamfile copy because they are identical..."
                ...
                if not os.path.dirname(receptor) == ligand_dir:
                    try:
                        ...
                        tkMessageBox.showerror("Error!", (
                                    "Impossible to copy the receptor\n%s\n\tto\n%s\n\nGIVING UP...\n(ERROR: %s)" % (
                            receptor, ligand_dir, sys.exc_info()[1])))
                        TheButton.config(state=DISABLED, text=" [ Generation aborted ]")
                        EnableInterface()
                        return False
                else:
                    if DEBUG: print
                    "TheFunction> skipped the source/dest receptor copy because they are identical..."
            ...
            # b. use cached maps
                ...
                if DEBUG: print
                "TheFunction> lig_atypes>", lig_atypes
            ...
            if docking_set.get() == "From template...":
                ...
                if not prepareDPFfast(dpf_file, receptor_filename=receptor, ligand_filename=ligand,
                                      lig_types=lig_atypes, flex_res_filename=flex_res,
                                      parameter_file=DPFParameterFile.get()):
                    tkMessageBox.showerror("Error!", (
                                "Impossible to create the DPF file:\n%s\n GIVING UP...\n(ERROR: %s)" % (
                        dpf_file, sys.exc_info()[1])))
...
DPF:\n%s\n\tto\n%s\n\nGIVING UP...\n(ERROR: %s)" % (DPFParameterFile.get(), ligand_dir, sys.exc_info()[1])))
                        ...
                        if DEBUG: print
                        "TheFunction> skipped the source/dest DPFparamfile copy because they are identical..."
            jobs_done += 1
            if splitting:
                counter += 1
    ...
    if not TarFile.get() == "[disabled]":
        ...
        if DEBUG:
            print
            "TheFunction> creating the package file", pack_filename, "in ", path
...
def CopyMapDir(atomtypes_to_copy, source_dir, destination_dir, symlink=False):
    ...
    if DEBUG: print
    "CopyMapDir> called with the following args", atomtypes_to_copy, source_dir, destination_dir, symlink
    if source_dir == destination_dir:
        if DEBUG: print
        "CopyMapDir> skipping copy/symlink because the directories are the same"
        return True
    if MapSource.get() == 2:
        ...
        for map in map_files:
            ...
            if atype == map.split(".")[-2]:
                try:
                    # if True:
                    if DEBUG: print
                    "CopyMapDir> going to process this file ==>", map
                    if symlink:
                        if DEBUG: print
                        "     doing symlinking", map, destination_dir
                        ...
                        if DEBUG: print
                        "     source (%s) => dest (%s) " % (map, destination_dir)
                        shutil.copy2(map, destination_dir)
                    counter = counter + 1  # +1 to account for the two maps.* files
                    # xxx.append(map)
                except:
                    print
                    "CopyMapDir> ERROR! ", exc_info()[1]
                    tkMessageBox.showerror(title="Cached maps error", message=(
                                ("Some problem occurred when copying or linking the file %s") % map))
                    return False
    # print "copied maps"
    # for x in xxx:
    #    print x
...
def CalcCacheMaps(output_dir, receptor, flexible_residues=None):
    ...
    if not os.path.dirname(receptor) == output_dir:
        try:
            ...
            if DEBUG: print
            "CalcCacheMaps> error in copying the receptor %s in %s" % (receptor, output_dir)
            tkMessageBox.showerror(title="Error", message=(
            ("Impossible to copy the receptor in the target directory for caching the maps.")))
        gpf_name = output_dir + os.sep + os.path.basename(receptor).rsplit('.', 1)[:-1][0] + "_all_maps.gpf"
    else:
        if DEBUG: print
        "CalcCacheMaps> skipping the receptor copy, because files are identical"
...
GPF:\n%s\n\tto\n%s\n\nGIVING UP...\n(ERROR: %s)" % (GPFParameterFile.get(), output_dir, sys.exc_info()[1])))
    ...
    if True:
        ...
        # except:
        if DEBUG: print
        "CalcCacheMaps> error in creating the GPF with the following parameters:", gpf_name, receptor, atom_types, flexible_residues
        return False
    if RunAutoGrid(output_dir, gpf_name):
        return True
    else:
        if DEBUG: print
        "CalcCacheMaps> error in running AutoGrid"
        return False
...
def RunAutoGrid(working_dir, gpf):
    DEBUG = True
    if DEBUG: print
    "RunAutoGrid> THE FUNCTION HAS BEN CALLED WITH THE FOLLOWING PARAMS", working_dir, gpf
    # define AutoGrid
    #
    if not AutoGridBin.get():
        if DEBUG: print
        "RunAutoGrid> Error, AutoGrid binary not found"
        return False
    else:
    ...
    if DEBUG:
        print
        "RunAutoGrid> files and paths report:"
        print
        "\tthe normalized path is |" + working_dir + "|"
        print
        "\t the gpf name is " + gpf + "|"
        print
        "\t the glg name is " + glg + "|"
    ...
    try:
        if system == "Windows":
            ...
            if DEBUG: print
            drive + " && cd " + directory + " && " + AutoGrid + " -p \"" + gpf + "\" -l \"" + glg + "\""
            os.system(drive + " && cd " + directory + " && " + AutoGrid + " -p \"" + gpf + "\" -l \"" + glg + "\"")
        else:
            if DEBUG: print
            "cd \"%s\"; %s -p \"%s\" -l \"%s\" " % (working_dir, AutoGrid, gpf, glg)
            os.system(("cd \"%s\"; %s -p \"%s\" -l \"%s\" " % (working_dir, AutoGrid, gpf, glg)))
    except:
        tkMessageBox.showerror("AutoGrid error!",
                               ("An error occurred when running AutoGrid. The generation will be halted."))
        return False
    ...
    try:
        ...
        if DEBUG: print
        "RunAutoGrid> Impossible to open the file ", glg
        ...
        if AutoGrid:
            ...
            if not AutoGridBin.get():
                answer = tkMessageBox.askquestion('Warning',
                                                  'The AutoGrid binary file is required for pre-caching maps.\nDo you want to define it?')
                if answer == "no":
                    tkMessageBox.showwarning("Pre-caching aborted",
                                             "The calculations of maps has been aborted by the user.")
                    keepasking = False
                    GPFframe.forget()
                    AutoGridWhen1.invoke()  # Select the default as "Run AG in each job"
                    AutoGridExecButton.config(text="Set AutoGrid executable", fg='red')
...
def genFlex(receptor_filename):
    ...
    if verbose or DEBUG: print
    "[genFlex] rigid will be   = ", rigid_filename
    if verbose or DEBUG: print
    "[genFlex] flexres will be = ", flexres_filename

    residue_selected = ListFlexResiduesNames.get()
    residue_selected = residue_selected.replace(" ", "")
    if verbose or DEBUG: print
    "[genFlex] gen from ", residue_selected

    residue_to_move = residue_selected.replace(',', '_')
    if verbose or DEBUG: print
    "These are residues to move", residue_to_move
    ...
    if True:
        ...
        if not bnd_pairs == ['']:
            ...
            for pair in bnd_pairs:
                ...
                all_bnds += bnds
        print
        "THIS IS GOING TO BE THE INPUT", all_res, rigid_filename, flexres_filename
        fdp = AD4FlexibleReceptorPreparation(r, residues=all_res, rigid_filename=rigid_filename,
                                             flexres_filename=flexres_filename, non_rotatable_bonds=all_bnds)
        return rigid_filename, flexres_filename
    # except:
    else:
        if DEBUG or verbose: print
        "genFlex> problems in generating the flex residues..."
        return False, False
...
def MakeMasterJobScript_multireceptor(path, list_of_receptor_dirs):
    ...
    try:
        ...
        if DEBUG: print
        "MakeMasterJobScript_multireceptor> ERROR generating the script:  %s " % filename
        tkMessageBox.showerror("Multi-receptor script.",
                               ("An error occurred when trying to generate the multi-receptor script file."))
        return False
...
_______/______/__////_/___///___/////__/////____/////___/////__/____/_______
    ...
    if TargetOS.get() == "lin":  # TODO useless!
        ...
        try:
            ...
            tkMessageBox.showerror("Master script generation.",
                                   ("An error occurred when trying to generate the jobs list file."))
            return False
    ...
    if TargetOS.get() == "pbs":
        ...
        try:
            ...
            tkMessageBox.showerror("PBS script generation.",
                                   ("An error occurred when trying to generate the jobs list file."))
            return False
...
def CreateSmuggler(path):
    ...
    try:
        ...
        tkMessageBox.showerror("PBS script generation.",
                               ("An error occurred when trying to generate the %s file." % filename))
        return False
...
""" % (version, full_date, outdir, tot_number_jobs, operative_system, machine, target_machine, str(HbDmin.get()),
       str(HbDmax.get()), str(HbAmin.get()), str(HbAmax.get()), str(MWmin.get()), str(MWmax.get()), str(NatMin.get()),
       str(NatMax.get()), str(TORSDOFmin.get()), str(TORSDOFmax.get()), str(DoRejectATypes.get()), ligand_count)
        ...
        if FlexResDefined.get():
            ...
            if DoFlexFromWhat.get() == 1:
                if FlexResFileName.get():  # TODO in theory it shouldn't be necessary
                    ...
                    if DEBUG: print
                    "RETURNING A SHAMEFUL FALSE: problems in logging the flex residues [FlexResFileName.get() = ", FlexResFileName.get(), "]"
                    return False
            if DoFlexFromWhat.get() == 2:
                if FlexResSelected.get():  # TODO in theory it shouldn't be necessary
                    ...
                    if DEBUG: print
                    "RETURNING A SHAMEFUL FALSE: problems in logging the flex residues [FlexResSelected.get() = ", FlexResSelected.get(), "]"
                    return False
    ...
    try:
        ...
        if DEBUG: print
        "problems in opening the log file"
...
def MakeTar(source_dir, tarfilename):
    if DEBUG:
        print
        "MakeTar> creating the tar from the source =>", source_dir
        print
        "MakeTar> The filename is    =>", tarfilename
    ...
    try:
        if DEBUG: print
        "Trying to generate the tar file", tarfilename
        ...
        if DEBUG: print
        "problems in creating the tar file"
        return False
...
def MakeZip(source_dir, zipfilename):
    ...
    for item in file_list:
        if DEBUG: print
        "Adding |%s| to %s" % (item, zipfilename)
...
def ImportLigList(filename=None):
    if not filename:
        filename = askopenfilename(title="Select a ligand list file......",
                                   filetypes=[("Ligands list", "*.lig"), ("Any file...", '*')])
        if not filename:
            return False
    try:
    ...
    if len(list) > 0:
        ...
        if len(found) == 0:
            tkMessageBox.showwarning("Ligand list",
                                     "No ligands loaded!\nCheck the list content...\n(maybe it's not a list)")
            return False
        if len(missing) > 0 and tkMessageBox.askyesno("Ligands imported",
                                                      "%d ligands have not been found.\n\nDo you want to inspect the list of rejected ligands?" % len(
                                                              missing)):
...
def ExportLigList():
    ...
    def ExportDone(filename=None):
        ...
        if len(list):
            if not filename:
                filename = asksaveasfilename(title="Select a ligand list file......",
                                             filetypes=[('Ligand list', '*.lig'), ("Any file...", "*")])
            if not filename:
                EnableInterface()
                return
    ...
    try:
        ...
        Radiobutton(SelectionLevel.interior(), text=accepted_msg, variable=SaveLig, value="accepted").grid(row=1,
                                                                                                           column=0,
                                                                                                           sticky=W,
                                                                                                           padx=15)
        Radiobutton(SelectionLevel.interior(), text=rejected_msg, variable=SaveLig, value="rejected").grid(row=2,
                                                                                                           column=0,
                                                                                                           sticky=W,
                                                                                                           padx=15)
...
AddLigandsButton = Button(LigandButtonsGroup, compound=LEFT, text='Add ligands', image=add_single_icon,
                          command=openLigand)
...
AddLigandsDirButton = Button(LigandButtonsGroup, compound=LEFT, text='Add directory', image=add_dir_icon,
                             command=openLigandDir)
...
AddLigandsDirRecursiveButton = Button(LigandButtonsGroup, compound=LEFT, text='Add recursively',
                                      image=add_dir_recursive_icon, command=lambda: openLigandDir(recursive=True))
...
LigandPDBQTOptButton = Button(p1, compound=LEFT, text="PDBQT generation options", image=pdbqt_lig_opt_icon,
                              command=LigandImportOptions)
...
RemoveLigandsButton = Button(LigandButtonsGroup, compound=LEFT, text='Remove selected', image=remove_selected_icon,
                             command=removeLigand)
...
RemoveAllLigandsButton = Button(LigandButtonsGroup, compound=LEFT, text='Remove all', image=remove_all_icon,
                                command=removeAllLigands)
...
AutoGridWhen1 = Radiobutton(AutoGridMenu.interior(), text='Run AutoGrid\nat every job', value=0, variable=MapSource,
                            highlightbackground='black', selectcolor='lightblue', command=MapMenu, indicatoron=False,
                            height=2, width=10)
...
AutoGridExecButton = Button(AGoptions.interior(), compound=LEFT, text="Set the AutoGrid executable",
                            image=open_smfolder_icon, command=GetAutoGrid)
...
# Summary_group.grid_columnconfigure(1, weight = 1)
Label(Summary_group.interior(), text="Ligands : ", font=('Helvetica', 12, 'bold')).grid(row=1, column=1, padx=5,
                                                                                        sticky=E)
Label(Summary_group.interior(), text="Receptor(s) : ", font=('Helvetica', 12, 'bold')).grid(row=2, column=1, padx=5,
                                                                                            sticky=E)
Label(Summary_group.interior(), text="Maps : ", font=('Helvetica', 12, 'bold')).grid(row=3, column=1, padx=5, sticky=E)
Label(Summary_group.interior(), text="Docking : ", font=('Helvetica', 12, 'bold')).grid(row=4, column=1, padx=5,
                                                                                        sticky=E)
SetOutDirButton = Button(Summary_group.interior(), text="Set directory...", command=SetJobDirectory,
                         image=open_smfolder_icon,
...
SystemButton1 = Radiobutton(Summary_group2.interior(), text='Workstation', value='lin', variable=TargetOS,
                            command=GetOSoption)
SystemButton1.grid(row=1, column=0, sticky=W + N, padx=5)
SystemButton2 = Radiobutton(Summary_group2.interior(), text='Linux cluster', value='pbs', variable=TargetOS,
                            command=GetOSoption)
